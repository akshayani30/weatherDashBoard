...
import requests               #to make the  http request and accept and open Meteo API
...
WMO_CODES={
    ...
    95: "Thunderstrom",
    96: "Thunderstrom with slight hail",
...
def fetch_weather(lat,lon):
    r=requests.get(
        ...
        params={"latitude":lat,"longitude":lon,"current":"temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,wind_direction_10m,precipitation,weathercode,uv_index",
                "hourly":"temperature_2m,precipitation_probability",
                "daily":"temperature_2m_max,temperature_2m_min,precipitation_sum",
                "timezone":"auto","forecast":7
                },timeout=8
    )
...
with st.spinner("Fetching weather data.."):  # it is for weather
    try:
...
def fmt(c):
    return round(c*9/5 +32,1) if unit== "Fanrenheit" else c
#current weather
st.divider()
